...
echo "export FRONT_END_URL=https://az-int.metrics.nsatc.net/" > EnvVariables.sh 
...
MyContainerId="$(sudo docker run -it --privileged --rm -d --network host --name monitor ghmccontainer.azurecr.io/monitor_ghpi:latest)"
  if [[ -z $MyContainerId ]]
  then
